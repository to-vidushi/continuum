...
export default function DailyWinsPage() {
  ...
  const toggleDone = async (win: Win) => {
    ...
    const { data } = await supabase
      .from('daily_wins')
      .update({ completed: !win.completed })
      .eq('id', win.id)
      .select()
    ...
    <div className={styles.page}>
        ...
        <div className={styles.modalOverlay} onClick={() => setShowScore(false)}>
          <div className={styles.modal} onClick={e => e.stopPropagation()}>
            ...
            <div className={styles.starsRow}>
              {[1,2,3,4,5].map(s => (
                <span key={s} style={{ fontSize: 36, color: s <= stars ? '#f5a623' : '#e0ddd8' }}>★</span>
              ))}
            </div>
            ...
            <div className={styles.scoreMessage}>{message}</div>
      ...
      <div className={styles.header}>
        <div className={styles.tabs}>
          ...
          {/* Today */}
          <button
            className={`${styles.tab} ${activeOffset === 0 ? styles.tabActive : ''}`}
          ...
          {/* Yesterday */}
          <button
            className={`${styles.tab} ${activeOffset === 1 ? styles.tabActive : ''}`}
          ...
          <div className={styles.dropdownWrapper}>
            <button
              className={`${styles.tab} ${activeOffset >= 2 ? styles.tabActive : ''}`}
          ...
          return (
            <div key={category} className={styles.column}>
              ...
              {catWins.map(win => (
                <div
                  key={win.id}
                  className={`${styles.card} ${win.completed ? styles.cardCompleted : styles.cardDefault}`}
                >
                  <div className={styles.cardTop}>
                    ...
                    {isToday && (
                      <button className={styles.deleteBtn} onClick={() => deleteWin(win.id)}>×</button>
                    )}
                  </div>
                  <div className={styles.cardBottom}>
                    <input
                      ...
                      onChange={() => toggleDone(win)}
                      disabled={!isToday}
                      className={`${styles.checkbox} ${!isToday ? styles.checkboxDisabled : ''}`}
                    />
                ...
                addingTo === category ? (
                  <div className={styles.addRow}>
                    <input
                      ...
                      onKeyDown={e => {
                        if (e.key === 'Enter') addWin(category)
